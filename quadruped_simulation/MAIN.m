...
% 0-trotto; 1-balzo; 2-passo 3-galoppo; 4-trotto veloce; 5-strisciare
gait = 5; 
p = get_params(gait);
p.playSpeed =1;
...
multiplot(t,X(:,1:3),Xd(:,1:3),'t [sec]','Position [m]',legenda,'position.pdf');

%%%%%%%%% velocity %%%%%%%%%
legenda={'$$v_x$$', '$$v_y$$', '$$v_z$$', '$$v_{x,d}$$', '$$v_{y,d}$$', '$$v_{z,d}$$'};
multiplot(t,X(:,4:6),Xd(:,4:6),'t [sec]','Velocity [m/s]',legenda,'velocity.pdf');

%%%%%%%%% Angular velocity %%%%%%%%%  
legenda={'$$\omega_x$$', '$$\omega_y$$', '$$\omega_z$$', '$$\omega_{x,d}$$', '$$\omega_{y,d}$$', '$$\omega_{z,d}$$'};
multiplot(t,X(:,16:18),Xd(:,16:18),'t [sec]','Angular velocity [rad/s]',legenda,'angvel.pdf');

%% control
real = [U2(:,3) U2(:,6) U2(:,9) U2(:,12)];
desired = [Ud(:,3) Ud(:,6) Ud(:,9) Ud(:,12)];
legenda = {'$$Fz_1$$', '$$Fz_2$$', '$$Fz_3$$', '$$Fz_4$$', '$$Fz_{1,d}$$', '$$Fz_{2,d}$$', '$$Fz_{3,d}$$', '$$Fz_{4,d}$$'};
singleplot(t2,t,real,desired,'t [sec]','Fz [N]',legenda,'force.pdf');
